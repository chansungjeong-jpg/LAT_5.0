...
LAT 5.0은 매수 여부를 대신 정해주는 시스템이 아니다. 돈이 들어오는 섹터 안에서,
확률 높은 눌림목 자리인지를 근거와 함께 정직하게 보여주는 관찰·필터 도구다.

```text
Watchlist 포함 = 매수 가능 아님. 분석 대상일 뿐.
Probability Score·Expected Value·CONDITIONAL_BUY 같은 확률/기댓값 기반
자동 매수신호는 만들지 않는다 (2장 참조).
RSI 등 보조지표는 수치만 표시하고 자동 판단·점수·게이트에 쓰지 않는다.
데이터가 불확실하면 UNKNOWN/REJECT로 멈춘다 — 판단을 지어내지 않는다.
최종 매수 판단은 항상 사용자가 차트와 함께 내린다.
```
...
## 0. 최우선 1차 필터 — 구현 불변 계약

LAT 5.0의 모든 종목 선정·추세선 탐지·스코어링보다 다음 1차 하드필터를
무조건 먼저 적용한다. 이 절의 우선순위는 뒤쪽의 일반 파이프라인 설명보다 높다.

```text
Core Watchlist 전체(현재 고유 종목 154개, 약 150개)
↓
월봉 SMA10 회복 AND 주봉 SMA5 회복
↓ 1차 필터 통과 종목만
60분 상승 추세선·EMA60/EMA120 돌파 탐지
↓
거래량·이격도·매물대·상위 시간봉 스코어링
↓
Hard Block 적용 및 최종 순위
```

### 0.1 회복 상태 계약

- 회복은 `직전 종가 < 직전 SMA`에서 `현재 종가 >= 현재 SMA`로 상향
  교차한 사건이다.
- 교차가 최근 3개 봉 안에서 발생했고 최신 봉 종가가 계속 SMA 이상이면
  `회복 유지`로 인정한다. 따라서 한 봉 전에 회복한 종목을 누락하지 않는다.
- 월봉은 SMA10, 주봉은 SMA5를 각각 사용하며 두 조건을 모두 충족해야 한다.
- 진행 중인 당월봉·당주봉을 오늘 기준 판단에 포함하되 `잠정 회복`으로 표시한다.
- 완료봉으로 확정된 회복은 `확정 회복`으로 표시한다.
- 데이터 부족·SMA 계산 불가·현재 SMA 재이탈은 fail-closed로 1차 필터에서
  제외하고 이유를 기록한다.

### 0.2 후속 분석 및 순위 계약

- 1차 필터 미통과 종목에는 추세선·이평선 돌파 점수나 최종 순위를 부여하지 않는다.
- 2차 신호는 완료된 60분봉의 확정 저점 2개를 연결한 상승 추세선과
  EMA60/EMA120 조건으로 탐지한다.
- 순위 점수는 추세선 20, 이평선 20, 거래량 20, 이격도 15, 매물대 15,
  월봉·주봉 10의 총 100점 상한을 사용한다.
- 이격도 과열·가까운 매물대·손익비 부족 등 Hard Block은 점수보다 우선한다.
- 통과 종목이 0개여도 기준을 자동 완화하지 않는다. 후보·항목별 점수·차단
  이유를 리포트에 남긴다.
...
Core Watchlist 전체
↓
월봉 SMA10 회복 AND 주봉 SMA5 회복 (최우선 1차 하드필터)
↓
KOSPI/KOSDAQ 주도주 발굴·랭킹
↓
60분 상승 추세선·EMA60/EMA120 돌파
↓
거래량·거래대금·이격도·매물대 점수
↓
Hard Block·Risk Reward
↓
BUY / WATCH / REJECT
```
...
거래량은 가장 높은 점수 비중을 가진다(2026-08-16 변경). 최근 완료 일봉 거래량을
직전 7거래일(당일 제외) 평균 거래량과 비교한 비율로 점수화한다 — 스마트머니
유입 여부를 단일 시점이 아니라 최근 평균 대비로 판단하기 위함이다. 일봉
거래대금은 이와 별개로 최소 유동성 확인용 Hard Block으로 사용한다.

```text
일봉 거래량 비율 = 최근 완료 일봉 거래량 / 직전 7거래일(당일 제외) 평균 거래량
<0.8=0, 0.8=5, 1.0=10, 1.2=15, 1.5=20, 2.0=30
```

직전 7거래일 데이터가 없으면(과거 이력 8거래일 미만) 비율은 계산하지 않고
거래량 점수는 0으로 처리한다. 구현: `src/lat5/location_decision.py`의
`_daily_volume_score`/`build_context`.
